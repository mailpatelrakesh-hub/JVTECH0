...
export default function CandidateModal({ 
  ...
  const callGeminiClientSide = async (base64Data: string, mimeType: string, apiKey: string): Promise<any> => {
    ...
    for (const modelName of modelsToTry) {
      try {
        console.log(`Attempting client-side Gemini extraction with model: ${modelName}`);
        ...
        lastError = err;
        console.error(`Error with model ${modelName} during client-side extraction:`, err);
    ...
    if (lastErrorMessage.includes("404") || lastErrorMessage.toLowerCase().includes("not found")) {
      ...
      throw new Error(`Gemini client-side extraction failed: ${lastErrorMessage}`);
    }
  };
